...
```{r, include=FALSE}
#To add appropriate column names and change "?" to "NA"
...
data <- adult_income %>% rename("age"=X1,"workclass"=X2,"fnlwgt"=X3,"education"=X4,
                        ...
                        "country"=X14,"income"=X15) %>%    #give appropriate column names
  mutate(income=recode(income, ">50K"="over_50K", "<=50K"="under_50K")) %>%
  mutate_at(vars("workclass","education","marital_status","occupation","relationship","race","sex","country"), na_if, "?")   #replace "?" with NAs in categorical variables
```
